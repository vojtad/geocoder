  ...
  class MaxmindGeolite2Database < Base
    ...
    def results(query)
      if configuration[:package] == :city
        q = "SELECT l.country_name, l.country_iso_code, l.subdivision_1_name, l.city_name, b.latitude, b.longitude
        ...
        format_result(q, [:country_name, :country_code2, :region_name, :city_name, :latitude, :longitude])
        ...
        WHERE b.network >>= inet '#{query.text}'"
        format_result(q, [:country_name, :country_code2, :latitude, :longitude])
      end
    end
